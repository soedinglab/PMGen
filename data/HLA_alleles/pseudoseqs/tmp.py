...
import jmp
import pickle  # added by Amir for debug
...
class AlphaFold(hk.Module):
    ...
    def __call__(
            ...
            ensemble_representations=False,
            return_representations=False,
            initial_guess=None):  # added by https://github.com/AmirAsgary
        ...
        if self.config.num_recycle:
            ...
            #### added by https://github.com/AmirAsgary
            if emb_config.initial_guess:
                # Check if prev_pos.shape[0] > initial_guess.shape[0]
                if prev_pos.shape[0] > initial_guess.shape[0]:
                    # Calculate padding size
                    pad_size = prev_pos.shape[0] - initial_guess.shape[0]
                    # Post-pad initial_guess along axis 0 to match prev_pos shape
                    initial_guess = jnp.pad(
                        initial_guess,
                        pad_width=((0, pad_size), (0, 0), (0, 0)),  # Pad only axis 0 (post-padding)
                        mode='constant',
                        constant_values=0  # Pad with zeros
                    )
                # Now add initial_guess to prev_pos (shapes should match)
                prev_pos += initial_guess
            #### before added by https://github.com/AmirAsgary
            prev = {
                'prev_pos': prev_pos,
...
class PredictedAlignedErrorHead(hk.Module):
    ...
    def loss(self, value, batch):
        ...
        true_affine = quat_affine.QuatAffine(quaternion=None, translation=batch['backbone_translation'],
                                             rotation=batch['backbone_rotation'], unstack_inputs=True)
        # Shape (num_res)
