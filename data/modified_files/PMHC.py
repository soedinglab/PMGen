    ...
    def __init__(self, id, peptide, allele_type=[], MHC_class = 'I',
        ...
        while self.top_rank_num <= self.max_num_rank:
            ...
            if MHC_class == 'I' and anchors == []:
                ...
                try:
                    self.fill_allele_seq_info(use_templ_seq=use_templ_seq, top_rank_num=self.top_rank_num)
                    print('DEBUGGGGG:', self.allele_type, self.top_rank_num)
        ...
        if not success:
            ...
            self.anchors = [1, len(self.peptide)] if self.MHC_class == 'I' else [1, 4, 6, 9]
            print(f'Therefore, using canonical anchors: {self.anchors}')
        if success:
            print(f'Anchors predicted for {self.id} using top_rank_num={self.max_num_rank}, allele={self.allele_type}')
        print('###############################################')
        self.info()
