...
class MyLoop(MA.loopmodel):
    ...
    def special_patches(self, aln):
        ...
        self.rename_segments(segment_ids=['M', 'B', 'P'], renumber_residues=[1, 1, 1])        #self.rename_segments(segment_ids=['M', 'B', 'P'], renumber_residues=[1, 1, 1])
        pass
    ...
    def select_loop_atoms(self):
        ...
        return M.selection(self.residue_range('3:P', '8:P'))# Be sure those arguments are always in the same line!
    ...
    def special_restraints(self, aln):
        rsr = self.restraints
        atoms = self.atoms
        modelling_stdev= float('0.1')  #STDEV MARKER   DO NOT DELETE THIS COMMENT

        # need to add more distance restraints later :)
        contact_file = open('contacts_myTestCase.list', 'r')
