...
class Target(PMHC):
    ...
    def __init__(self, id, peptide, allele_type=[], MHC_class = 'I',
                 ...
                 rm_netmhcpan_output=True, reverse=False, max_num_rank=20):
        ...
        Args:
            ...
            max_num_rank: (int, optional) number of times to try to predict anchor with netMHCpan with top n blast searched alleles.
        ''' 
        ...
        self.reverse = reverse
        self.max_num_rank = max_num_rank
        self.anchors = anchors
        ...
        while self.top_rank_num <= self.max_num_rank:
            ...
            if MHC_class == 'I' and len(anchors) < 2:
                ...
                try:
                    ...
                    if not self.anchors:
                        raise ValueError(f'no anchor predicted via netMHCpan and allele {self.allele_type}, '
                                         f'we should use the next similar allele:'
                                         f'{self.top_rank_num}')
                    print('Predicted anchors:', self.anchors)
                    success = True
                ...
                try:
                    ...
                    print(
                        f'Error: Something went wrong when predicting the anchors using netMHCIIpan (top_rank_num={self.top_rank_num})')
            elif MHC_class == 'II' and len(anchors) == 4: 
                success=True
                given_by_user=True
            elif MHC_class == 'I' and len(anchors) == 2: 
                success=True
                given_by_user=True

            # Try the next top_rank_num if it fails
            self.top_rank_num += 1
            if success: break

        if not success:
            print(f'Failed to predict anchors even after trying up to top_rank_num={self.max_num_rank}')
            self.anchors = [2, len(self.peptide)] if self.MHC_class == 'I' else [1, 4, 6, 9]
            print(f'Therefore, using canonical anchors: {self.anchors}')
        if success:
            if not given_by_user: print(f'Anchors predicted for {self.id} using top_rank_num={self.max_num_rank}, allele={self.allele_type}')
            else: print('Anchors Provided By User and will be used in modeling:', anchors)
        print('###############################################')
        self.info()
    ...
    def info(self):
        ...
        print('ID: %s' % self.id)
        print('Type: MHC class %s' % self.MHC_class)
        print(f'Top allele number {self.top_rank_num} Selected')
        print('Alleles: %s' % self.allele_type)
        if self.M_chain_seq != '':
            print('Alpha chain length: %s' %len(self.M_chain_seq))
        if self.N_chain_seq != '' and self.MHC_class == 'II':
    ...
    def fill_allele_seq_info(self, use_templ_seq=False, top_rank_num=1): #### DEBUG top_rank_num
        ...
        Args:
            ...
            only_use_top_template (int, optional): Choose which top rank blast resulted template to use.
