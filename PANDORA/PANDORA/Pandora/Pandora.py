...
def constrained_alignment(seq1, seq2, pos_seq1, pos_seq2, match=1, mismatch=-1, gap=-2):
    ...
    segments = []
    prev1, prev2 = -1, -1
    for p1, p2 in zip(pos_seq1, pos_seq2):
        segments.append((seq1[prev1 + 1:p1], seq2[prev2 + 1:p2]))  # Before anchor
        segments.append((seq1[p1], seq2[p2]))  # Anchor point
        prev1, prev2 = p1, p2
...
def naive_align(s1, s2, match, mismatch, gap):
    # Placeholder for actual alignment logic
    # For simplicity, just pad with gaps if lengths differ
    if len(s1) < len(s2):
        s1 += "-" * (len(s2) - len(s1))
    elif len(s2) < len(s1):
        s2 += "-" * (len(s1) - len(s2))
    return s1, s2
######### added by Amir, ends here

class Pandora:

    def __init__(self, target, database=None, template=None, no_modelling=False): # added by Amir --> no_modelling
        ...
        Args:
            ...
            no_modelling (bool): When active, modelling will not happen and only files are generated.
    ...
    def find_template(self, best_n_templates=1, benchmark=False, verbose=True):
        ...
        if not type(self.template)==list:
            self.template = [self.template]
        # if condition added by Amir
        #if self.no_modelling:
        self.no_modelling_output_dict['template_anchors'] = []
        self.no_modelling_output_dict['target_anchors'] = []
        self.no_modelling_output_dict['template_id'] = []
        self.no_modelling_output_dict['aln_M'] = []
        self.no_modelling_output_dict['aln_B_N'] = []
        self.no_modelling_output_dict['aln_P'] = []
        self.no_modelling_output_dict['aln_template'] = []
        self.no_modelling_output_dict['aln_target'] = []
        self.no_modelling_output_dict['template_path'] = []
        for tmp in self.template:
            self.no_modelling_output_dict['template_anchors'] += [tmp.anchors]  # list
            self.no_modelling_output_dict['target_anchors'] += [self.target.anchors]
            self.no_modelling_output_dict['template_id'] += [tmp.id]  # str
            assert (isinstance(tmp.M_chain_seq, str) and len(tmp.M_chain_seq)) != 0, f'tmp.M_chain_seq should be a string, found {tmp.M_chain_seq}'
            aln_M = pairwise2.align.globalxx(self.target.M_chain_seq, tmp.M_chain_seq)[0][:2]
            if self.target.MHC_class == 'II':
                assert (isinstance(tmp.N_chain_seq, str) and len(tmp.N_chain_seq)) != 0, f'tmp.N_chain_seq should be a string, found {tmp.N_chain_seq}'
                aln_B_N = pairwise2.align.globalxx(self.target.N_chain_seq, tmp.N_chain_seq)[0][:2]
            elif self.target.MHC_class == 'I':
                assert (isinstance(tmp.B2M_seq, str) and len(tmp.B2M_seq)) != 0, f'tmp.B2M_seq should be a string, found {tmp.B2M_seq}'
                aln_B_N = [''.join(['-'] * len(tmp.B2M_seq)), tmp.B2M_seq]
            assert isinstance(self.target.peptide, str), f'target.peptide not a string, found: {self.target.peptide}'
            aln_P = constrained_alignment(self.target.peptide, tmp.peptide, [i - 1 for i in self.target.anchors], [i - 1 for i in tmp.anchors])
            aln_template = aln_M[1] + aln_B_N[1] + aln_P[1]
            aln_target = aln_M[0] + aln_B_N[0] + aln_P[0]
            self.no_modelling_output_dict['aln_M'] += [aln_M]
            self.no_modelling_output_dict['aln_B_N'] += [aln_B_N]
            self.no_modelling_output_dict['aln_P'] += [aln_P]
        ...
        if isinstance(self.template, list): self.template = self.template[0]
    ...
    def model(self, n_loop_models=20, n_homology_models=1,
        ...
        print('SELECTED TEMPLATE: ', self.template.id)
        if self.no_modelling: # added by Amir, if and else here for modelling mode and no modelling mode
            print('**No modelling being performed in PANDORA due to no_modelling mode**')
