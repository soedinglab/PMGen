...
from model4_recon import build_reconstruction_model
...
AA_DIM      = 21                      # 20 aa + MASK token

def onehot(seq: str, max_len: int) -> np.ndarray:
    """Return one-hot with an explicit MASK channel (all-zeros for padding)."""
    arr = np.zeros((max_len, AA_DIM), dtype=np.float32)
    for i, aa in enumerate(seq[:max_len].upper()):
        arr[i, AA_TO_IDX.get(aa, MASK_IDX)] = 1.0
    return arr

def mask_random_positions(oh: np.ndarray, mask_rate: float = .3):
    """
    Replace ~mask_rate positions by MASK token.
    Returns (masked_input, y_true, sample_weight)
    """
    pep_len = oh.shape[0]
    mpos    = np.random.rand(pep_len) < mask_rate
    y_true  = oh.copy()
    sample_weight = mpos.astype(np.float32)          # 1 on masked positions

    masked = oh.copy()
    masked[mpos] = 0.0
    masked[mpos, MASK_IDX] = 1.0
    return masked, y_true, sample_weight
...
# ---------------------------------------------------------------------
def _row_to_tensor_pack(row_dict: dict,
                        max_pep_seq_len: int,
                        max_mhc_len: int,
                        mask_rate: float = 0.3):
    # peptide -----------------------------------------------------------
    pep_oh = onehot(row_dict["long_mer"], max_pep_seq_len)
    pep_masked, y_true, sw = mask_random_positions(pep_oh, mask_rate)

    # MHC latent --------------------------------------------------------
    mhc = _read_embedding_file(row_dict["mhc_embedding_path"]).astype("float32")
    if mhc.shape[0] != max_mhc_len:
        raise ValueError("MHC length mismatch")

    return (pep_masked, mhc), y_true, sw     # <-- three-tuple
...
def create_streaming_dataset(parquet_path: str,
    ...
    """
    output_signature = (
        (tf.TensorSpec((max_pep_seq_len, AA_DIM), tf.float32),
         tf.TensorSpec((max_mhc_len, 1152), tf.float32)),
        tf.TensorSpec((max_pep_seq_len, AA_DIM), tf.float32),  # y_true
        tf.TensorSpec((max_pep_seq_len,), tf.float32),  # sample_weight
    )
    ...
    ds = ds.interleave(
            lambda features, y_true, sw: tf.data.Dataset.from_tensors((features, y_true, sw)),
...
def plot_training_curve(history: tf.keras.callbacks.History, run_dir: str, fold_id: int = None,
    ...
    if model is not None and val_dataset is not None:
        ...
        y_true = []
        for _, labels, _ in sample_dataset:
...
def main(argv=None):
    ...
    for fold_id, ((train_loader, val_loader), class_weight) in enumerate(zip(folds, class_weights), start=1):
        ...
        model, attn_model = build_reconstruction_model(max_peptide_length, max_mhc_length)
        model.summary()
        ...
        # Verify data shapes
        for (x_pep, latents), labels, sw in train_loader.take(1):
