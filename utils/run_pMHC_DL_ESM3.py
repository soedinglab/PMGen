...
def _load_parquet_samples(parquet_path, seq_len):
    pq_file = pq.ParquetFile(parquet_path)
    for batch_df in pq_file.iter_batches(batch_size=BUFFER):
        df = batch_df.to_pandas()
        for _, row in df.iterrows():
            # Convert peptide sequence to one-hot encoding
            one_hot_peptide = peptides_to_onehot(row["long_mer"], seq_len)

            # Load MHC embedding
            mhc_embedding = _read_embedding_file(row["mhc_embedding_path"])

            # Ensure MHC embedding is a numpy array
            if not isinstance(mhc_embedding, np.ndarray):
                raise ValueError(f"Expected MHC embedding to be a numpy array, got {type(mhc_embedding)}")

            # Yield the sample as a tuple
            yield (one_hot_peptide, mhc_embedding), int(row["assigned_label"])
...
def make_stream_dataset(
    ...
    output_signature = (
        (
            tf.TensorSpec(shape=(max_pep_seq_len, 21), dtype=tf.float32),  # No batch dim
    ...
    ds = tf.data.Dataset.from_generator(
        lambda: _load_parquet_samples(parquet_path, max_pep_seq_len),
...
def plot_test_metrics(model, test_dataset, run_dir: str, fold_id: int = None, history=None, string: str =None):
    ...
    """
    # CORRECTED LOGIC: Extract labels first.
...
def main(argv=None):
    ...
    p.add_argument("--model_name", default=2)
    ...
    for fold_id, ((train_loader, val_loader), class_weight) in enumerate(zip(folds, class_weights), start=1):
        ...
        model = build_classifier(longest_peptide_seq_length, max_mhc_seq_length)
        model.summary()
...
if __name__ == "__main__":
    ...
    main([
        ...
        "--epochs", "3", "--batch", "128", "--test_batches", "200",
    ])