...
BATCH_SIZE = 40
NUM_EPOCHS = 50
NUM_EPOCH_SAMPLES = 200000  # Number of random samples to use per epoch (set to None to use all data)
LEARNING_RATE = 1e-4
MIXED_PRECISION = False  # Set to True to enable mixed precision training (float16)
...
CHECKPOINT_PATH = '/home/amir/amir/ParseFold/PMGen/outputs/good_structures_mutscreen_structure_pred/arrays/checkpoints/20251226-180124/best_model.weights.h5'   # Path to checkpoint file (.weights.h5). If None, uses best_model.weights.h5 from latest run
...
class BatchedDataLoader:
    ...
    def get_batches(self, batch_size, shuffle=True, num_samples=None):
        """
        ...
        Args:
            ...
            num_samples: If provided, only use this many random samples per epoch.
    ...
    def get_num_batches(self, batch_size, num_samples=None):
        """
        Get number of batches per epoch.
        
        Args:
            batch_size: Number of samples per batch
            num_samples: If provided, calculate based on this many samples.
                        If None, use all samples.
        """
        total_samples = num_samples if num_samples is not None else self.num_samples
        total_samples = min(total_samples, self.num_samples)
        return (total_samples + batch_size - 1) // batch_size


def train_batched():
    """
    Train the mutation structure predictor model.
    
    Uses configuration from Script Arguments section:
        - RESUME_TRAINING: If True, resume training from a checkpoint
        - CHECKPOINT_PATH: Path to checkpoint file (.weights.h5) to resume from.
                          If None and RESUME_TRAINING=True, will look for latest checkpoint
    """
    print("=" * 60)
    ...
    # Use config from Script Arguments
    batch_size = BATCH_SIZE
    num_epochs = NUM_EPOCHS
    num_epoch_samples = NUM_EPOCH_SAMPLES
    ...
    print(f"  Samples per epoch: {num_epoch_samples if num_epoch_samples else 'All'}")
    ...
    print(f"Loaded {len(paired_inds)} total sample pairs")
    
    # Create data loader
    data_loader = BatchedDataLoader(coords, seqs, paired_inds, n_atoms, cutoff=DATA_CUTOFF, pad_token=PAD_TOKEN)
    num_batches = data_loader.get_num_batches(batch_size, num_samples=num_epoch_samples)
    ...
    model = MutationStructurePredictorAtt(
        ...
        heads=NUM_HEADS, num_layers=NUM_LAYERS, cutoff=MODEL_CUTOFF, PAD_TOKEN=float(PAD_TOKEN)
    )
    ...
    best_loss = 0.875118 #  0.875118
    recent_losses = []
    global_step = 0
    start_epoch = 0
    
    # Create timestamped run directory for checkpoints
    run_timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    checkpoint_dir = os.path.join(OUTPUT_DIR, 'checkpoints', run_timestamp)
    ...
    if RESUME_TRAINING:
        ...
        if checkpoint_path is None:
            checkpoints_base = os.path.join(OUTPUT_DIR, 'checkpoints')
            if os.path.exists(checkpoints_base):
                # Find the latest run directory
                run_dirs = sorted([d for d in os.listdir(checkpoints_base) 
                                  if os.path.isdir(os.path.join(checkpoints_base, d))])
                if run_dirs:
                    latest_run = run_dirs[-1]
                    checkpoint_path = os.path.join(checkpoints_base, latest_run, 'best_model.weights.h5')
                    training_state_path = os.path.join(checkpoints_base, latest_run, 'training_state.json')
                    print(f"Found latest run: {latest_run}")
        else:
            # Derive training state path from checkpoint path
            checkpoint_dir_from_path = os.path.dirname(checkpoint_path)
            training_state_path = os.path.join(checkpoint_dir_from_path, 'training_state.json')
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            dummy_input = {
                'coords': tf.zeros([1, 10, n_atoms, 3], dtype=tf.float32),
                'wt_indices': tf.zeros([1, 10], dtype=tf.int32),
                'mut_indices': tf.zeros([1, 10], dtype=tf.int32),
                'mutation_mask': tf.zeros([1, 10], dtype=tf.float32),
                'atom_mask': tf.ones([1, 10, n_atoms], dtype=tf.float32),
                'res_indices': tf.zeros([1, 10 * n_atoms], dtype=tf.int32),
            }
            ...
            if training_state_path and os.path.exists(training_state_path):
                ...
                print(f"Restored training state: epoch={start_epoch}, global_step={global_step}, best_loss={best_loss:.6f}")
        else:
    ...
    # TensorBoard - use same timestamp as checkpoints
    log_dir = os.path.join(OUTPUT_DIR, 'logs', run_timestamp)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (inputs, true_coords, atom_mask) in enumerate(
            data_loader.get_batches(batch_size, shuffle=True, num_samples=num_epoch_samples)
        ):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # Save training state for epoch checkpoint
            training_state = {
                'epoch': epoch + 1,
                'global_step': global_step,
                'best_loss': float(best_loss)
            }
            with open(os.path.join(checkpoint_dir, f'training_state_epoch_{epoch+1}.json'), 'w') as f:
                json.dump(training_state, f, indent=2)
...
if __name__ == "__main__":
    if DATA_PREP:
        parse_and_align_pdb_all(input_dir='/home/amir/amir/ParseFold/PMGen/outputs/bad_structures_mutscreen_structure_pred/structures', 
                        reference_dir=REFERENCE_PATH,
                          output_dir='/home/amir/amir/ParseFold/PMGen/outputs/bad_structures_mutscreen_structure_pred/arrays', 
                          PAD_TOKEN=-2)
    else:
        train_batched()