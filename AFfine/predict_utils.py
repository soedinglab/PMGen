...
import af2_util # added by https://github.com/AmirAsgary
...
def initial_guess_features(query_seq, template_pdb_path, template_sequence, aln=None, anchors=None, peptide_seq=None):
    all_positions, all_positions_mask = af2_util.get_atom_positions_initial_guess(template_pdb_path, query_seq,
                                                                                  template_sequence,
                                                                                  aln, anchors, peptide_seq)
    initial_guess = af2_util.parse_initial_guess(all_positions)
...
def run_alphafold_prediction(
        ...
        dump_pdbs=True,
        dump_metrics=True,
        template_pdb_dict=None, # added by Amir to add template pdb files for initial_guesss
    ...
    # Below, query_sequence, template_pdb_dict, no_initial_guess and return_all_outputs added by Amir
    all_metrics = predict_structure(
        out_prefix, feature_dict, model_runners, crop_size=crop_size,
        dump_pdbs=dump_pdbs, dump_metrics=dump_metrics, query_sequence=query_sequence,
         template_pdb_dict=template_pdb_dict, no_initial_guess=no_initial_guess,
...
def predict_structure(
        ...
        dump_pdbs=True,
        dump_metrics=True,
        query_sequence=None, #added by https://github.com/AmirAsgary/
        template_pdb_dict=None, #added by https://github.com/AmirAsgary/
        no_initial_guess=False,  #added by https://github.com/AmirAsgary/
        return_all_outputs=False
):
    ...
    for model_name, model_runner in model_runners.items():
        ...
        if not no_initial_guess:
            # get the best template and initialize alphafold with it
            template_pdb_list = template_pdb_dict['template_path']
            aln = (template_pdb_dict['aln_target'][0], template_pdb_dict['aln_template'][0])
            anchors = template_pdb_dict['target_anchors'][0]
            peptide_seq = template_pdb_dict['aln_P'][0][0].replace('-','') #[n][0] target, [n][1] template
            assert len(template_pdb_list) > 0
            initial_guess = initial_guess_features(query_sequence, template_pdb_path=template_pdb_list[0],
...
def load_model_runners(
        ...
        small_msas = True,
        args = None # added by https://github.com/AmirAsgary
):
