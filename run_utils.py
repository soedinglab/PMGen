...
class run_PMGen_modeling():
    def __init__(self, peptide, mhc_seq, mhc_type, id, output_dir='output',
        ...
        Args:
            ...
            benchmark_exclude_ids (bool): Exclude benchmark ids in given df in run_PMGen.py
        """
        super().__init__()
            ...
            with open(log_file, 'w') as f, contextlib.redirect_stdout(f), contextlib.redirect_stderr(f):
                try:
                    ...
                    if self.benchmark and getattr(case, 'similarity_info', None) is not None:
                        # Source of truth: no_modelling_output_dict['template_id'] (all templates actually used).
                        # Fall back to case.template if no_modelling_output_dict isn't populated.
                        nmod = getattr(case, 'no_modelling_output_dict', {}) or {}
                        selected_ids = list(nmod.get('template_id') or [])
                        if not selected_ids:
                            tmpl_attr = case.template
                            templates_list = tmpl_attr if isinstance(tmpl_attr, (list, tuple)) else [tmpl_attr]
                            selected_ids = [t.id for t in templates_list]
                        ...
                        info = {
                            ...
                            'template_similarities': template_sims,
                            'n_templates_below_threshold': case.similarity_info['n_templates_below_threshold'],
                        }
    ...
    def _aggregate_benchmark_similarity_csv(self):
        ...
        for info in rows:
            d = {
                ...
                "n_templates_below_threshold": info["n_templates_below_threshold"]
            }
...
class run_proteinmpnn():
    def __init__(self, PMGen_pdb, output_dir,
                ...
                peptide_chain='P', mhc_design=True, peptide_design=True,
                 ...
                 model_name='v_48_020', only_multichain=True):
        '''
        Args:
            ...
            only_multichain (bool): Only does multi-chaining and not running proteinmpnn.
        '''
        self.pdb = PMGen_pdb
        self.output_dir = output_dir
        ...
        self.only_multichain = only_multichain
        self.input_assertion()
...
class MultipleAnchors:
    def __init__(self, args, dirty_mode=False):
        ...
        self.use_netmhcpan = not getattr(args, 'no_netmhcpan', False)
        self.only_last_anchor = getattr(args, 'only_last_anchor', False)
        if self.only_last_anchor:
            assert not self.use_netmhcpan, '--only_last_anchor requires --no_netmhcpan to be set'
    ...
    def _process_row(self, row):
        ...
        if self.use_netmhcpan:
            try:
                ...
                pass  # fall through to anchor combinations below

        # Either netmhcpan is disabled or it failed: enumerate all possible anchor combinations
    ...
    def process(self):
        ...
        try:
            ...
            _ = df['mhc_seq']  # verify columns parsed correctly
        except:
            df = pd.read_csv(self.args.df)        
        print(f" Starting Multiple Anchor Mode on {self.args.max_cores} cores. netMHCpan {'enabled' if self.use_netmhcpan else 'disabled (using all anchor combinations)'}.")
